...
public class FBUtilities implements Cloneable {
	...
	transient static Scanner in = new Scanner(System.in);
	 /*
	 * EXPLANATION:
	 *  Using a Stack will work best for the Undoer. 
	 *  This will allow access to the last action done, 
	 *  which will always be the top of the stack.
	 */
	static Stack<Undoer> Undoer = new Stack<Undoer>();
	...
	public static void addUser(ArrayList<FacebookUser> fbUser) {
		...
		FacebookUser newUser = new FacebookUser(name,pass,pHint);
		fbUser.add(newUser);
		Undoer.push(new Undoer(1,newUser,null));
	...
	public static void deleteUser(ArrayList<FacebookUser> fbUser) {
		...
		for(int i=0; i<fbUser.size(); i++) {
			...
			found = true;
			FacebookUser deletedUser = fbUser.get(i);
			fbUser.remove(deletedUser);
			Undoer.push(new Undoer(2,deletedUser,null));
			}
		}
		if(!found) {
			System.out.println("ERROR! Username not found!");
	...
	public static void addFriend(ArrayList<FacebookUser> fbUser) {
		...
		for(int i=0; i < fbUser.size(); i++) {
			if(fbUser.get(i).getUsername().equals(tempName)) {
				...
				Undoer.push(new Undoer(3,fbUser.get(i),fbUser.get(j)));
			}
		}
		if(!found) {
			System.out.println("ERROR! Username not found!");
	...
	public static void removeFriend(ArrayList<FacebookUser> fbUser) {
		...
		for(int i=0; i < fbUser.size(); i++) {
			if(fbUser.get(i).getUsername().equals(tempName)) {
				...
				Undoer.push(new Undoer(4,fbUser.get(i),fbUser.get(j)));
			}
		}
		if(!found) {
			System.out.println("ERROR! Username not found!");
	...
	public static void like(ArrayList<FacebookUser> fbUser) {
		...
		if(fbUser.get(j).friends.isEmpty()) {
			...
			found = true;
			Undoer.push(new Undoer(5,null,fbUser.get(j)));
		}
		else {
			fbUser.get(j).like(like);
			found = true;
	...
	public static void Undo(ArrayList<FacebookUser> fbUser) {
		FacebookUser u;
		int i;
		Undoer undo = Undoer.peek();
		if(Undoer.empty()) {
			System.out.println("Nothing to undo!");
		}
		switch(undo.getAction()) {
		case 1:
			fbUser.remove(undo.getValue());
			System.out.println("Undoing 'Add a User'...");
			break;
		case 2:
			fbUser.add(undo.getValue());
			System.out.println("Undoing 'Delete a User'...");
			break;
		case 3:
			System.out.println(undo.getUser().toString() + " needs to log in first...");
			i = login(fbUser);
			u = fbUser.get(i);
			if(u==null || u!=undo.getUser()) {
				System.out.println("ERROR! Undo unsuccessful...");
			}
			u.defriend(undo.getValue());
			System.out.println("Undoing 'Add a Friend'...");
			break;
		case 4:
			System.out.println(undo.getUser().toString() + " needs to log in first...");
			i = login(fbUser);
			...
			u.friend(undo.getValue());
			System.out.println("Undoing 'Remove a Friend'...");
			break;
		case 5:
			System.out.println(undo.getUser().toString() + " needs to log in first...");
			i = login(fbUser);
			...
			u.likes.remove(u.likes.size()-1);
			System.out.println("Undoing 'Add a Like'...");
			break;
		}
		Undoer.pop();
	}//end of Undo method
